...
==========================================================
Extension 模式: 接收 body_html/body_text → pipeline → 存储 + 返回
CDP 模式: 通过 cdp_handler 捕获 (兼容保留)
持久化: 每次捕获自动写入 SQLite, 支持历史查询
"""
...
def api_capture():
    ...
    # ── 来源1: Extension (有 body_html 或 body_text) ──
    if data.get("body_html") or data.get("body_text"):
        return _capture_from_extension(data)

    # ── 来源2: CDP 标签页 (有 tab_id) ─────────────────
    if data.get("tab_id"):
        return _capture_from_cdp(data["tab_id"])

    return jsonify({"error": "缺少 body_html/body_text 或 tab_id"}), 400
...
def _capture_from_extension(data: dict):
    ...
    try:
        ...
        log.exception("Pipeline 处理失败")
        return jsonify({"error": f"处理失败: {e}"}), 500


def _capture_from_cdp(tab_id: str):
    """兼容旧 CDP 捕获方式"""
    try:
        from cdp_handler import capture_tab
        result = capture_tab(tab_id)
    except ConnectionError as e:
        log.error(f"CDP 连接丢失: {e}")
        return jsonify({
            "error": "Chrome 连接已断开",
            "reconnecting": True,
        }), 503
    except Exception as e:
        log.exception("CDP 捕获失败")
        return jsonify({"error": f"捕获失败: {e}"}), 500

    if "error" in result:
        return jsonify(result), 500

    # CDP 处理: 模态框文本直接用, 页面 HTML 需清洗
    if result.get("modal_content"):
        ...
        cleaned = process_content(result["page_html"])
        result["text"] = cleaned["markdown"]
        result["plain_text"] = cleaned["plain_text"]

    result["captured_via"] = "cdp"
    # 持久化到 SQLite (失败不影响捕获结果)
    _persist_capture(
        url=result.get("url", ""),
        ...
        captured_via="cdp", result=result,
    )
    return jsonify(result)
